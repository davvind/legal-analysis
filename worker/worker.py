...
Én enkelt databasetilkobling per jobb. Ingen separate tilkoblinger
i hjelpefunksjoner — alt går gjennom conn som åpnes én gang i
process_job() og sendes inn som parameter overalt.
"""
...
# ── Én tilkobling — åpnes i process_job, sendes inn overalt ──────────────────

def open_db():
    """
    Åpner én PostgreSQL-tilkobling. Løser DNS selv til IP for å unngå
    Windows TIME_WAIT-konflikt med psycopg2/libpq.
    """
...
# ── Alle DB-funksjoner tar conn som første parameter ─────────────────────────
...
def insert_claim(conn, page_id: str, run_id: str, speaker: str, speaker_role: str,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            (page_id, run_id, speaker, speaker_role, claim_text, normalized_claim,
             claim_text, topic, event_date, confidence)
        )
    conn.commit()
...
def get_blob_url(blob_path: str) -> str:
    """Genererer en SAS-URL for midlertidig tilgang til filen i Blob Storage."""
    from azure.storage.blob import generate_blob_sas, BlobSasPermissions
    from datetime import timedelta
    from urllib.parse import urlparse
    import re

    parsed = urlparse(f"https://dummy?{AZURE_STORAGE_CONNECTION_STRING.replace(';', '&')}")
    # Hent AccountName og AccountKey fra connection string
...
def extract_with_azure(pdf_path: str, blob_path: str = None) -> list[dict]:
    """
    Bruker URL-metoden for store filer (>4 MB) — Azure DI henter filen
    direkte fra Blob Storage via SAS-URL i stedet for bytes i requesten.
    """
...
def process_job(job_payload: dict):
    ...
    # ÉN tilkobling for hele jobben — åpnes her, sendes inn til alle funksjoner
    conn = open_db()
    print(f"[{job_id}] DB-tilkobling OK")

    try:
        update_job(conn, job_id, status="processing", current_step="henter_fil",
        ...
        # 2. Azure DI OCR — henter direkte fra Blob via SAS-URL
        update_job(conn, job_id, current_step="ocr", progress_pct=15)
        azure_run_id = start_processing_run(conn, "ocr", "azure-di-prebuilt-read")
        try:
            pages = extract_with_azure(None, blob_path=blob_path)
        ...
        # 4. Claim-ekstraksjon
        update_job(conn, job_id, current_step="claim_ekstraksjon", progress_pct=45)
        claim_run_id = start_processing_run(conn, "claim_extraction", "gpt-4o-mini", "v1")
        claims_count = 0
        for idx, page in enumerate(pages):
            for c in extract_claims_from_page(page["text"]):
                try:
                    insert_claim(
                        conn, pg_page_ids.get(page["page_num"]), claim_run_id,
                    ...
                    continue
            if idx % 20 == 0:
                update_job(conn, job_id, progress_pct=min(45 + int(20 * idx / max(len(pages), 1)), 65))
        ...
        update_job(conn, job_id, current_step="chunking_embedding", progress_pct=70)
        prefix_context = (
        ...
        for sub_idx, subdoc in enumerate(subdocs):
            ...
            for chunk in build_chunks(subdoc["sider"], MAX_WORDS_FIN, "fin", ctx) + \
        ...
        update_job(conn, job_id, status="failed", error_message=str(e)[:1000], completed=True)

...
def db():
    ...
    print(f"[DEBUG] db(): løser DNS for {hostname}...")
    ip = socket.gethostbyname(hostname)
    print(f"[DEBUG] db(): IP er {ip}, forsøker tilkobling...")
    ...
    try:
        ...
        print(f"[DEBUG] db(): tilkobling OK")
        return conn
    except Exception as e:
        print(f"[DEBUG] db(): FEILET med {type(e).__name__}: {e}")
        raise
...
def process_job(job_payload: dict):
    ...
    # Åpne delt tilkobling FØR update_job() — unngår Windows TIME_WAIT-konflikt
    # der to raske tilkoblinger til samme server feiler pga. port-gjenbruk.
    conn = db()

...
def db():
    ...
    MERK: psycopg2 på Windows kan feile med 'Address already in use' ved
    tilkobling via hostnavn til Azure Postgres, sannsynligvis pga.
    IPv6/IPv4-håndtering i libpq, OG pga. at azure-servicebus sin aktive
    AMQP-tilkobling kan interferere med Windows sin socket-allokering for
    påfølgende tilkoblinger fra samme prosess. Vi løser DNS selv og legger
    inn en kort pause + eksplisitt retry for å unngå kollisjon.
    """
    import socket
...
def process_job(job_payload: dict):
    ...
    # ÉN delt tilkobling for hele jobben (IP-oppløst, se db() for forklaring)
    conn = db()

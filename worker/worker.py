...
def update_job(job_id: str, status: str = None, progress_pct: int = None,
    ...
    try:
        with conn.cursor() as cur:
            ...
            if started:
                sets.append("started_at = %s"); params.append(datetime.utcnow())
            if completed:
                sets.append("completed_at = %s"); params.append(datetime.utcnow())
...
def start_processing_run(run_type: str, model_name: str, prompt_version: str = "") -> str:
    conn = db()
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def insert_chunk(case_id: str, file_id: str, page_id_start: str, page_id_slutt: str,
...
def get_previous_part_last_paragraph(pdf_id: str, del_nummer: int) -> str:
    ...
    try:
        ...
        for page in pages:
            pg_page_ids[page["page_num"]] = insert_page(
                file_id, page["page_num"], page["text"], page["dokumentid"], azure_run_id
            )

        # 4. Claim-ekstraksjon
        update_job(job_id, current_step="claim_ekstraksjon", progress_pct=45)
        claim_run_id = start_processing_run("claim_extraction", "gpt-4o-mini", "v1")
        claims_count = 0
        ...
        complete_processing_run(claim_run_id, "completed", f"{claims_count} claims")
        ...
        prefix_context = get_previous_part_last_paragraph(pdf_id, del_nummer) if pdf_id and del_nummer > 1 else ""
        subdocs = segment_into_subdocs(pages)

        embed_run_id = start_processing_run("embedding", EMBED_MODEL)
        chunks_total = 0
        ...
        for sub_idx, subdoc in enumerate(subdocs):
            ...
            for chunk in fine_chunks + grov_chunks:
                ...
                insert_chunk(
                    case_id, file_id, page_id_start, page_id_slutt,
        ...
        complete_processing_run(embed_run_id, "completed", f"{chunks_total} chunks")
